...
describe('normalizeSearchInput', () => {
  it('translates a /carte map URL into the canonical /recherche query', () => {
    ...
    expect(params.get('locations')).toBe(
      'Clermont-Ferrand__45.785791226648406_3.093987259418718_6650',
    );
    ...
    expect(query.get('locations')).toBe(
      'Clermont-Ferrand__45.785791226648406_3.093987259418718_6650',
    );
  ...
  it('detects map view from lat/lng even without a /carte path', () => {
    const { isMap, params } = normalizeSearchInput(
      'category=9&lat=45.78&lng=3.09&defaultRadius=5000&city=Clermont-Ferrand',
      BASE,
    );
    expect(isMap).toBe(true);
  ...
  it('accepts a path+query form', () => {
    const { params, isMap } = normalizeSearchInput(
      'recherche?category=15&text=mac',
      BASE,
    );
    expect(isMap).toBe(false);
...
describe('buildQueryString', () => {
  it('injects the category id only when absent', () => {
    expect(buildQueryString(new URLSearchParams('a=1'), '9')).toContain(
      'category=9',
    );
    expect(buildQueryString(new URLSearchParams('category=2'), '9')).toContain(
      'category=2',
    );
  });
});
